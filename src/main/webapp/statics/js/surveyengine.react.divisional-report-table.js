...
const path = '/reports/get/divisional-report'
const months = ['', 'JANUARY', 'FEBRUARY', 'MARCH', 'APRIL', 'MAY', 'JUNE',
    'JULY', 'AUGUST', 'SEPTEMBER', 'OCTOBER', 'NOVEMBER', 'DECEMBER']
const divisions = ["EMS", "PWB", "ISS/PMC", "ENC", "BPA/CAB"]

class DivisionalReport extends React.Component {
    constructor() {
        super();
        this.state = {
            data: [],
    ...
    getRatingColumn = (row) => {
        ...
        if(row.value.length > 0){
            divisions.forEach(division => {
                ...
                divisionalColumn.push(<div className="rt-td" style={rating && rating.answer.length > 0 && this.state.showComments ? {height: '11%'}: {height: '35'}}><div className={rating && rating.answer ? ratingColumn == 'Comment' ? 'Comment' : rating.answer.replace(' ', '-') : ratingColumn != 'Comment' ? 'NA-rating': ''}><div className={`rating-divisional-${division.substr(0,3)}`}>{rating && rating.answer && ratingColumn == 'Comments' ? rating.answer : ratingColumn == 'Comments' ? '':division}</div></div></div>);
            })
        }
                    ...
                    columns= {[
                        {
                            Header: "Quality",
                            accessor: "answers",
                            maxWidth: this.state.showComments ? 85 : 225,
                        ...
                        },
                        {
                            Header: "Contact",
                            accessor: "recipient",
                            Cell: row => <div className="rating-divisional-EMS">{row.value}</div>
                        },
                        {
                            Header: "Comment",
                            accessor: "answers",
                            minWidth: this.state.showComments ? 400 : 0,
                            Cell: row => this.getRatingColumn(row),
            ...
            <div className={`tab-content col-lg-12 clearfix`}>
                                ...
                                <div className="col-xs-12">
                                    <div className="col-lg-8 col-xs-12 padding-0">
                                        <div className="pull-left mtop-10" style={{fontSize: 14}}>
                                            <div className="wrap"> This is the customer satisfaction overview detail by division for {months[this.state.fiscalMonth]} {this.state.fiscalYear}. (unsubmitted ratings are not shown here)</div>
                                        </div>
                                    </div>
                                    <div className="col-lg-4 col-xs-12 padding-0">
                                        <div className="pull-right">
                ...
                <div className="mtop-40">
                    <div className="col-xs-11">
                        ...
                        <span className="col-lg-2 col-sm-3 col-xs-12"><div className="legend-circle Needs-improvement col-xs-3"></div>  1 - Needs improvement</span>
                        <span className="col-lg-2 col-sm-3 col-xs-12"><div className="legend-circle Acceptable col-xs-3"></div>  2 - Acceptable</span>
                        <span className="col-lg-2 col-sm-3 col-xs-12"><div className="legend-circle Good col-xs-3"></div>  3 - Good</span>
                        <span className="col-lg-2 col-sm-3 col-xs-12"><div className="legend-circle NA-rating col-xs-3"></div>  N/A</span>
                        <span className="col-lg-2 col-sm-3 col-xs-12"><div className="legend-circle Awaiting col-xs-3"></div>  Awaiting Response</span>
                    </div>
                </div>
            </div>
        );
...
ReactDOM.render(<DivisionalReport/>, document.getElementById('report-table'));
